...
rootPath = uigetdir('/media/Test_Data/','Please select the root path');
listing = dir(rootPath);

for i = 3:length(listing)
    clearvars -except rootPath listing i;
    if listing(i).isdir == 0
        continue;
    end
    level1 = listing(i).name;
    if exist([rootPath,'/',level1,'/SignalAsciiData/DT_',level1,'_TEMA_Displacement.txt'],'file')
        inFile = ['DT_',level1,'_TEMA_Displacement.txt']
        inPath = [rootPath,'/',level1,'/SignalAsciiData/'];
    else
        continue;
    end
