...
[dtFile,dtPath] = uigetfile('/media/Test_Data/DT*.csv','Please select the drop tower data file');
...
triggerSignal = droptower(:,12);
indexes = find(triggerSignal < 4,1,'first')-1-samplingRate/5: find(triggerSignal < 4,1,'first')-1+samplingRate/2; % trigger - 200ms -> trigger + 500 ms
trigger = triggerSignal(indexes);
time = -200:1/samplingRate*1000:500; % 1 to 500 ms

% droptower six axis loadcell
sixAxisDC = mean(droptower(1:indexes(1)-1,5:10)); % calculate the DC offset
...
save([dtPath,processedDataFileName],'time','sixAxis','oneAxis','pStrain1','pStrain2','phi','sixAxisCalibration','oneAxisCalibration','readme','trigger');
